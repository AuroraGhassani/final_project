...
export const getMyFollowingStories = async (page = 1, size = 10) => {
    ...
    try {
       const response = await axios.get(`${baseUrl}/api/v1/following-story?page=${page}&size=${size}`, config);
...
export const getMyStories = async (size = 10, page = 1) => {
    ...
    try {
        const response = await axios.get(`${baseUrl}/api/v1/my-story?size=${size}&page=${page}`, config);
